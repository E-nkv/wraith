...
func TestVocabSetRejectsUnknownGeneralAndNumbers(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "available: voice-type, numbero, none") {
	...
	if got := workspaceLoad(); got != "numbero" {
		t.Errorf("workspace = %q, want numbero", got)
	}
...
func TestPrintConfigCountsTheActiveVocabulary(t *testing.T) {
	...
	if !strings.Contains(out.String(), "vocabulary: 5 terms (general + numbero)") {
