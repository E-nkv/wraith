# AGENTS.md — v4 (deprecated)

Guidance for AI agents working in `v4/`, the deprecated Chrome-based version.
**v5 is the current project and lives at the repository root** with its own
`AGENTS.md`; its rules (Go, gofmt, uinput, OpenRouter) do not apply here, and
these do not apply there. v4 is kept installable but is no longer developed —
do not change it unless a task names v4 explicitly.
